...
#elif defined(ESP32)
	uint32_t deviceID = ESP.getEfuseMac() & 0xffffff;
#endif
uint32_t sensorID = deviceID;
...
void setup()
{
	...
	Serial.printf("deviceID (based on ESP): %06X\n", deviceID);
	Wire.begin();
	if (!bme.begin()) {
  ...
  CRC32 crc;
  uint32_t checksum = CRC32::calculate(bme.compensationParameters(test), 32);
  Serial.printf("SensorID (CRC32): 0x%08X  Hash(CRC32) of ", checksum);
  for (int i=0; i<32; i++)
  {
